    ...
    try {
        ...
        if (contentType.includes('application/json')) {
            ...
            if (typeof cursorsStr === 'string') {
                try { cursors = JSON.parse(cursorsStr) } catch (e) { cursors = {} }
            }
        ...
        if (!accountId || !action || !accessToken) {
            console.error('❌ [ASSETS] Missing fields:', { accountId, action, accessToken: accessToken ? 'PRESENT' : 'MISSING' })
        ...
        if (action === 'LIST') {
            ...
            const normalizedVideos = videosData.map((vid: any) => {
            ...
            let allAssets = [...dedupedImages, ...normalizedVideos].sort((a, b) => {
