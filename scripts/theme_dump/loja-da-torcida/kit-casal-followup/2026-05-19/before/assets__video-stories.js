document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (videoType === 'shopify' && shopifyVideoSrc) {
      ...
      contentHTML = `
        <video autoplay playsinline webkit-playsinline loop class="story-video">
    ...
    if (currentVideoElement) {
      ...
      function attemptPlay(video, withSound) {
        video.muted = !withSound;
        var p = video.play();
        if (p && typeof p.catch === 'function') {
          p.catch(() => {
            if (withSound) {
              // Autoplay com som bloqueado — fallback para mudo
              video.muted = true;
              updateMuteIcon(true);
              var p2 = video.play();
              if (p2 && typeof p2.catch === 'function') {
                p2.catch(() => {
                  // Último recurso: esperar loadeddata
        ...
        if (!withSound) updateMuteIcon(true);
      }

      attemptPlay(currentVideoElement, true);
    }
  });
