  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "relative h-svh bg-transparent will-change-[width]",
      ...
      <motion.div
        animate={{
          width: state === "collapsed" ? "var(--sidebar-width-icon)" : "var(--sidebar-width)",
          x: state === "collapsed" && variant === "floating" ? 0 : 0
        }}
        transition={{ duration: 0.2, ease: "easeInOut" }}
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh will-change-[width,transform] md:flex",
          side === "left"
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-none px-2 text-sidebar-foreground outline-none ring-sidebar-ring aria-disabled:pointer-events-none aria-disabled:opacity-50 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
