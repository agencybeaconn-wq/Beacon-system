/**
 * Threads — linhas fluidas de dados (React Bits, adaptado pra TSX).
 * Fonte: github.com/DavidHDev/react-bits (Backgrounds/Threads).
 * Adaptações NODE: mouse na window (container pointer-events:none),
 * guarda de prefers-reduced-motion. Já vem com cap de resolução e
...
export default function Threads({
    ...
    useEffect(() => {
        ...
        // Mouse na window: o container fica pointer-events:none pra não roubar cliques dos CTAs.
        function handleMouseMove(e: MouseEvent) {
            const rect = container.getBoundingClientRect();
            const x = (e.clientX - rect.left) / rect.width;
            const y = 1.0 - (e.clientY - rect.top) / rect.height;
            if (x >= 0 && x <= 1 && y >= 0 && y <= 1) targetMouse = [x, y];
            else targetMouse = [0.5, 0.5];
        }
        ...
        function update(t: number) {
            ...
            const { color: c, amplitude: a, distance: d, enableMouseInteraction: m } = propsRef.current;
    ...
    return <div ref={containerRef} className={className} style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden', ...style }} />;
}
