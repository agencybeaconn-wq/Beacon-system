...
export default function ClientConnectionsPage() {
    ...
    if (!selectedClientId) {
        ...
        if (saved) {
            return <Navigate to={`/clients/${saved}?tab=connections`} replace />;
        }
        return (
            <div className="flex flex-col items-center justify-center h-[calc(100vh-200px)] gap-4 text-center px-4">
                ...
                <p className="text-muted-foreground max-w-md">
                    Use o dropdown no cabeçalho para selecionar um cliente.
                </p>
            </div>
        );
    }

    return <Navigate to={`/clients/${selectedClientId}?tab=connections`} replace />;
}
