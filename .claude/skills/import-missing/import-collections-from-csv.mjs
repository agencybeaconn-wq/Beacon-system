...
async function main() {
  ...
  // 2) Adicionar produtos a cada coleção (não usa REST collects, usa GraphQL collectionAddProductsV2)
  console.log(`\n→ Adicionando produtos às coleções...`);
  let okColls = 0, failColls = 0;
  for (const [handle, items] of Object.entries(collectsByHandle)) {
    const coll = existing[handle];
    if (!coll) { console.warn(`  ! ${handle} não criada, pulando`); continue; }
    if (!coll.isCustom) { console.warn(`  ! ${handle} é smart collection (rules), pulando`); continue; }

    const productIds = items.map(it => it.productId);
    try {
      await addProductsToCollection(creds.shop, creds.token, coll.id, productIds);
      // Espera job assíncrono e tenta reordenar
      await delay(800);
      try {
        await reorderCollectionProducts(creds.shop, creds.token, coll.id, productIds);
      } catch (e) {
        // reorder pode falhar se job ainda não acabou — não fatal
        console.warn(`    ${handle}: reorder skip (${e.message.slice(0, 60)})`);
      }
      okColls++;
      console.log(`  + ${handle}: ${productIds.length} produtos`);
      await delay(500);
    } catch (e) {
      failColls++;
      console.error(`  ✗ ${handle}: ${e.message.slice(0, 120)}`);
    }
  }

  console.log(`\n=== Resultado ===`);
  console.log(`Coleções: criadas ${toCreate.length}, com produtos ${okColls}, falhas ${failColls}`);
