...
class MenuDrawer extends HTMLElement {
  ...
  bindEvents() {
    // Remove old listeners first to prevent duplicate stacking
    this.removeEventListeners();
    this._summaryHandler = this.onSummaryClick.bind(this);
    this._closeButtonHandler = this.onCloseButtonClick.bind(this);
    this.querySelectorAll('summary').forEach((summary) =>
      summary.addEventListener('click', this._summaryHandler)
    );
    this.querySelectorAll(
      'button:not(.localization-selector):not(.country-selector__close-button):not(.country-filter__reset-button)'
    ).forEach((button) => button.addEventListener('click', this._closeButtonHandler));
  }

  removeEventListeners() {
    if (this._summaryHandler) {
      this.querySelectorAll('summary').forEach((summary) =>
        summary.removeEventListener('click', this._summaryHandler)
      );
    }
    if (this._closeButtonHandler) {
      this.querySelectorAll(
        'button:not(.localization-selector):not(.country-selector__close-button):not(.country-filter__reset-button)'
      ).forEach((button) => button.removeEventListener('click', this._closeButtonHandler));
    }
  }
  ...
  onSummaryClick(event) {
    ...
    if (detailsElement === this.mainDetailsToggle) {
      ...
      const parentMenu = detailsElement.closest('.has-submenu') || detailsElement.closest('.mobile-facets__main');
      if (parentMenu) parentMenu.classList.add('submenu-open');
    }
  }
  ...
  closeMenuDrawer(event, elementToFocus = false) {
    ...
    this.mainDetailsToggle.querySelectorAll('.submenu-open').forEach((el) => {
  ...
  closeAnimation(detailsElement) {
    const animationTarget = detailsElement.querySelector('.menu-drawer') || detailsElement.querySelector('.menu-drawer__submenu');
    if (!animationTarget) {
    ...
    // Fallback in case transition doesn't trigger or is reduced
    setTimeout(() => {
...
class HeaderDrawer extends MenuDrawer {
  ...
  openMenuDrawer(summaryElement) {
    ...
    // before applying the transition class
...
class SliderComponent extends HTMLElement {
  ...
  initPages() {
    ...
    this.slidesPerPage = Math.round(this.slider.clientWidth / this.sliderItemOffset);
    // Formula for Step-based paging: Total steps = Total items - items visible + 1
  ...
  update() {
    ...
    this.nextButton.disabled = scrollLeft + sliderWidth >= scrollWidth - 5;

    // Step-based logic: identifier the current 'scroll page' starting from 1
    this.currentPage = Math.max(1, Math.min(
      Math.round(scrollLeft / this.sliderItemOffset) + 1,
      this.totalPages
    ));

    // Refinement: If next is disabled, we are at the last possible step (Total - Visible + 1)
  ...
  onButtonClick(event) {
    ...
    const nextPosition = event.currentTarget.name === 'next'
      ? this.slider.scrollLeft + step * this.sliderItemOffset
      : this.slider.scrollLeft - step * this.sliderItemOffset;

    this.slideScrollPosition = Math.max(0, Math.min(nextPosition, maxScroll));
