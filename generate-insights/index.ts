...
serve(async (req: Request) => {
    ...
    try {
        ...
        if (settings?.primary_kpis && Array.isArray(settings.primary_kpis) && settings.primary_kpis.length > 0) {
            settings.primary_kpis.forEach((kpi: any) => {
                if (kpi.metric === 'ROAS') {
        ...
        const campaignsUrl = `https://graph.facebook.com/${META_API_VERSION}/act_${accountId}/insights?` +
        ...
        for (const campaign of campaignInsights) {
            ...
            if (campaign.action_values) {
                campaign.action_values.forEach((value: any) => {
                    if (value.action_type === 'purchase' ||
                        value.action_type === 'omni_purchase' ||
                        value.action_type === 'onsite_conversion.purchase') {
        ...
        const adsUrl = `https://graph.facebook.com/${META_API_VERSION}/act_${accountId}/ads?` +
            `fields=id,name,status,campaign_id,campaign{name},adset_id,adset{name},url_tags,creative{url_tags}` +
